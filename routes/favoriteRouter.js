...
favoriteRouter
  ...
  .get(cors.cors, authenticate.verifyUser, async (req, res, next) => {
    try {
      ...
      const dish = favorite.dishes.find(
        (checkDish) => checkDish.id === req.params.dishId
      );
      if (!favorite) {
        res.statusCode = 200;
        res.setHeader("Content-Type", "application/json");
    ...
    async (req, res, next) => {
      try {
        const favorites =
          ...
          (await Favorites.create({ user: req.user.id }));
        const getNewDish = favorites.dishes.every(
          (dish) => dish.id.toString("hex") !== req.params.dishId
        );
        if (getNewDish) {
