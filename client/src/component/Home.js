...
import {
  ...
  menuTitleStyle,
  menuImgStyle,
  menuPlateLeftArrowStyle,
  menuPlateRightArrowStyle,
  dishContainerStyle,
  itemStyle,
  color,
} from '../style/Style';
...
const Home = ({ dishId, dishName, dishImg, viewDish, promoImg, nextOne }) => (
  <div className='Home' style={homeStyle}>
    ...
    <div
      id='aboutus'
      style={{ ...itemStyle, backgroundColor: color.backgroundColor }}
    >
      <div style={{ fontSize: '250%', fontWeight: 'bold', margin: '80px 0px' }}>
        About Us
      </div>
      ...
      <div className='promo-container' style={promoContainerStyle}>
        <img
          ...
          onError={(e) => {
            ...
            e.target.src = promoFallbackImg;
          }}
          alt='img break'
          style={{ width: 600, height: 400 }}
        />
        <Link to='/promos' className='icon' style={{ textDecoration: 'none' }}>
          <div style={promoIconStyle}>Go!</div>
        </Link>
      </div>
    </div>
  );
...
const HomeMenu = Radium(({ nextOne, viewDish, dishId, dishImg, dishName }) => {
  return (
    ...
    >
      <Link to='/menu' className='title' style={{ textDecoration: 'none' }}>
      ...
      <div className='menu-plate' style={menuPlateStyle}>
        ...
        <div className='dish-container' style={dishContainerStyle}>
          {viewDish.map((i) => (
            <div className='dish' key={i}>
              ...
              <div
                className='dish-name'
                style={{ fontSize: '70%', paddingTop: 10 }}
              >
                {dishName[i]}
              </div>
            </div>
        ...
        <div
          ...
          id='right'
          key='right'
          style={menuPlateRightArrowStyle}
        ></div>
      </div>
    </div>
  );
